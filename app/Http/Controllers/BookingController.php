...
class BookingController extends Controller
{
    ...
    */
    public function index(Request $request)
    {
        ...
        $userLogin = Auth::user();
        if($userLogin->id == 519){
            return redirect()->route('booking-bbc.index');
        }
        ...
        if($id_search){
           //  dd($id_search);
            ...
            if(Auth::user()->id == 515){//cano nguyen hien
                $arrSearch['xe_4t'] = $xe_4t = 0;
                $query->where('xe_4t', 0);
            }else{                
                if($xe_4t){    
                    $arrSearch['xe_4t'] = $xe_4t;            
            ...
            if($userLogin->beach_id == 1 || $userLogin->id == 494){
                $beach_ids = [7];
            }
            if($beach_ids){
            ...
            if($created_at){
                ...
                if($time_type == 1){ // theo thangs
                    ...
                    if($beach_ids){
                      $queryCostOther->whereIn('beach_id', $beach_ids);
                    }
        ...
        foreach($allList as $item){
            ...
            //tính chi phi hoa hong cho cano
            if(!isset($arrHoahong[$item->use_date][$item->beach_id])){
                $arrHoahong[$item->use_date][$item->beach_id] = 0;
            }       
            $arrHoahong[$item->use_date][$item->beach_id] += $item->commision;
            ...
            foreach($item->details as $detail){               
                ...
                if($detail->cate_id == 71) // cam 500 JohnTour
                {
                    ...
                    if(!isset($arrTongTienKhongCam[$item->use_date][$item->beach_id])){
                        $arrTongTienKhongCam[$item->use_date][$item->beach_id] = 0;
                    } 
                    $arrTongTienKhongCam[$item->use_date][$item->beach_id] += $detail->total_price;
                }
      ...
      //  dd($arrData['chup_anh']);
        ...
        if(!empty($arrHoahong)){
            foreach($arrHoahong as $date_use => $beachArr){
                if($date_use >= '2024-06-01'){
                    foreach($beachArr as $beach_id => $total_money){
                        $arrDataCost = [
                        ...
                        $rs = Cost::where(['beach_id' => $beach_id, 'cate_id' => 57, 'date_use' => $date_use])->first();
                        if(!$rs){
            ...
            foreach($arrTongTienKhongCam as $date_use => $beachArr){
                if($date_use >= '2024-06-01'){
                    foreach($beachArr as $beach_id => $total_money){
                        ...
                        $rs = Cost::where(['beach_id' => $beach_id, 'cate_id' => 58, 'date_use' => $date_use])->first();
                        if(!$rs){
                            Cost::create($arrDataCost);
                        }else{ 
                            //kiem tra tien da luu co bang tien tong hay khong thi update
        ...
        return view($view, compact( 'items', 'arrSearch', 'keyword', 'time_type', 'month', 'year', 'month_do', 'arrData', 'chi_tien_mat', 'chietkhauList', 'chi_khac', 'beach_id', 'cateList', 'cate_id', 'use_date_from_format', 'use_date_to_format', 'beachList', 'beachArr', 'partners', 'partner_id', 'cateArr', 'tuvanList'));
    ...
    */
    ...
    public function create(Request $request)
    {
        ...
        if($user->id == 494){ // bai bang
            $view = "booking.add-baibang";
        }
    ...
    */
    public function edit($id, Request $request)
    {
        ...
        if($detail->beach_id == 7){
            $view = 'booking.edit-baibang';
        }
        $tuvanList = NguoiTuVan::where('status', 1 )->orderBy('display_order')->get();
        return view($view , compact( 'detail', 'cateList','chietkhauList', 'partners', 'beachList', 'tuvanList'));
